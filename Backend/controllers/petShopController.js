...
const petShopRegController = async (req, res) => {
  try {
    ...
    if (alreadyPetShop) {
      return res
        .status(409)
        .json({ message: "PetShop already registered for this owner!" });
    }
    ...
    if (!aadharUpload) {
      return res
        .status(500)
        .json({ message: "Server error while uploading the Aadhar image!" });
    }
    ...
    if (!newPetShop) {
      return res
        .status(202)
        .json({ message: "Error occurred while creating the PetShop!" });
    }
    ...
    return res
      ...
      .json({
        message:
          "Thanks for registering with us. Your PetShop has been created successfully! An admin will verify your details shortly.",
      });
  } catch (error) {
...
const petShopLoginController = async (req, res) => {
  try {
    ...
    if (!petshop) {
      return res
        .status(404)
        .json({ message: "Petshop not found for this user!" });
    }
    ...
    if (!token) {
      return res
        .status(500)
        .json({ message: "Some Server Error, Please try again!" });
    }
...
const petShopUpdateController = async (req, res) => {
  try {
    ...
    const updatePetshop = await PetShop.findByIdAndUpdate(
      petshopId,
      {
        // FallBack Logic
    ...
    ); // Return the updated document and validate fields

    if (!updatePetshop) {
      return res
        .status(500)
        .json({ message: "Error while updating petshop details!" });
    }
    return res
      .status(200)
      .json({ message: "Petshop Details Updated Successfully!" });
  } catch (error) {
...
module.exports = {
  petShopRegController,
  petShopLoginController,
  petShopDeleteController,
  petShopUpdateController,
};
